...
async function checkBookingItemsSchema() {
    console.log('--- BookingItems SCHEMA ---');
    try {
        const { data } = await supabase.from('BookingItems').select().limit(1);
