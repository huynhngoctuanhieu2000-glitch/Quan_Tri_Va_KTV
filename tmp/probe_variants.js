...
async function probe() {
  ...
  const statuses = [
    'confirmed', 'Confirmed', 'dispatched', 'DISPATCHED', 'Dispatched', 'RESERVED', 'reserved'
  ];
  
  console.log(`Testing variants on real ID: ${realId}...`);
  ...
  for (const s of statuses) {
    ...
    if (error && error.message.includes('invalid input value for enum')) {
      ...
      // Revert
      await supabase.from('Bookings').update({ status: 'NEW' }).eq('id', realId);
    }
  }
}
