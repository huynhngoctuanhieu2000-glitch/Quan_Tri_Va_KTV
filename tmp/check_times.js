...
async function checkBookingsTime() {
    console.log('--- BOOKINGS TIME CHECK ---');
    try {
        const { data, error } = await supabase
            .from('Bookings')
            .select('id, billCode, timeBooking, createdAt, status')
            .order('createdAt', { ascending: false })
            .limit(10);
        if (error) throw error;
        
        data.forEach(b => {
            console.log(`Bill: ${b.billCode}, timeBooking: "${b.timeBooking}", createdAt: ${b.createdAt}`);
        });
    } catch (err) {
        console.error(err);
    }
