...
-- =================================================================
-- 1. Cập nhật hàm promote_next_assignment
-- Lỗi: Hàm này tự động set TurnQueue.status = 'waiting' khi KTV hết việc.
-- Sửa: Nếu status đang là 'off' thì giữ nguyên 'off'.
-- =================================================================
...
BEGIN
    ...
    IF NOT FOUND THEN
        ...
        RETURN jsonb_build_object('success', true, 'message', 'No next assignment, KTV set to waiting/off');
    END IF;
    ...
    UPDATE "TurnQueue"
    SET
        "status" = CASE WHEN "status" = 'off' THEN 'off' ELSE 'assigned' END, -- Sửa tại đây
...
-- =================================================================
-- 2. Cập nhật hàm dispatch_confirm_booking
-- Lỗi: Hàm này cũng tự động set TurnQueue.status = 'waiting' khi KTV bị xóa khỏi đơn hàng.
-- Sửa: Nếu status đang là 'off' thì giữ nguyên 'off'.
-- =================================================================
...
BEGIN
    ...
    BEGIN
        ...
        LOOP
            ...
            UPDATE "TurnQueue"
            SET "status" = CASE WHEN "status" = 'off' THEN 'off' ELSE 'waiting' END, -- Sửa tại đây
                "current_order_id" = NULL,
                "booking_item_id" = NULL,
                "booking_item_ids" = ARRAY[]::text[],
                "room_id" = NULL,
    ...
    LOOP
        ...
        IF NOT EXISTS (
            ...
            SET
                "status" = CASE WHEN "TurnQueue"."status" = 'working' THEN 'working' WHEN "TurnQueue"."status" = 'off' THEN 'off' ELSE 'assigned' END,
