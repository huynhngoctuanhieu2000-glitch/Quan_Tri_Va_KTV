-- ============================================================================
-- FIX: dispatch_confirm_booking — Xóa TurnLedger khi gán lại KTV (reassignment)
-- 
-- ROOT CAUSE:
--   Khi Lễ tân gán nhầm KTV (ví dụ NH021) rồi sửa lại (gán NH014),
--   block cleanup (section 0.5) CHỈ xóa KtvAssignments + TurnQueue cho KTV cũ,
--   nhưng KHÔNG xóa bản ghi TurnLedger.
--   → syncTurnsForDate() đếm TurnLedger → NH021 vẫn bị tính turns_completed = 1.
--
-- FIX:
--   Thêm DELETE FROM "TurnLedger" trong block cleanup.
--   Chỉ xóa nếu KTV CHƯA bắt đầu làm (TurnQueue.status != 'working').
--   Nếu KTV đã bắt đầu làm (working), GIỮ TurnLedger để tính công.
-- ============================================================================
