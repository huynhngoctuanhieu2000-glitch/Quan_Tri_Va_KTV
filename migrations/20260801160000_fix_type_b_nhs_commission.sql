-- Migration: Cập nhật logic tính tiền tua: KTV Loại B chỉ tính 180k/h cho mã NHP và NHT, còn lại 100k/h
-- Date: 2026-08-01
...
DECLARE
    v_work_type text;
    v_milestones_type jsonb;
    v_rate_60_type numeric := 100000;
...
BEGIN
    ...
    -- Determine work_type
    SELECT work_type INTO v_work_type FROM "Staff" WHERE id = p_staff_id;
    IF v_work_type IS NULL THEN v_work_type := 'TYPE_A'; END IF;

    -- Load TYPE_B configs if needed
    v_milestones_type := v_milestones;
    v_rate_60_type := v_rate_60;
    IF v_work_type = 'TYPE_B' THEN
        SELECT value INTO v_milestones_type FROM "SystemConfigs" WHERE key = 'ktv_commission_milestones_TYPE_B';
        IF NOT FOUND OR v_milestones_type IS NULL THEN
            SELECT value INTO v_milestones_type FROM "SystemConfigs" WHERE key = 'ktv_commission_milestones_type_b';
            IF NOT FOUND OR v_milestones_type IS NULL THEN v_milestones_type := v_milestones; END IF;
        END IF;

        SELECT value::text INTO v_config_val FROM "SystemConfigs" WHERE key = 'ktv_commission_per_60min_TYPE_B';
        IF FOUND AND v_config_val IS NOT NULL THEN
            v_rate_60_type := REPLACE(v_config_val, '"', '')::numeric;
        END IF;
    END IF;
    ...
    LOOP
        ...
        IF NOT v_has_items THEN
            ...
            BEGIN
                IF v_work_type = 'TYPE_B' THEN
                    -- KHÔNG BIẾT mã dịch vụ là gì (do fallback), mặc định cho hưởng 100k
                    v_active_milestones := v_milestones;
                    v_active_rate := v_rate_60;
                END IF;

                IF v_active_milestones ? '60' THEN
                    v_total_commission := v_total_commission + (v_active_milestones->>'60')::numeric;
                ELSE
                    v_total_commission := v_total_commission + v_active_rate;
                END IF;
            END;
        END IF;
    ...
    LOOP
        ...
        LOOP
            ...
            BEGIN
                ...
                IF v_active_milestones ? v_ktv_duration::text THEN
                    v_turn_amount := v_turn_amount + (v_active_milestones->>v_ktv_duration::text)::numeric;
                ELSE
                    v_turn_amount := v_turn_amount + (ROUND((v_ktv_duration::numeric / 60) * v_active_rate / 1000) * 1000);
                END IF;
            END;
        END LOOP;
