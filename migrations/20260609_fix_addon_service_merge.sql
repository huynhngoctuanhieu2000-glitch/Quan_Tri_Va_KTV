-- ============================================================================
-- FIX: dispatch_confirm_booking — Hỗ trợ gộp dịch vụ mua thêm (Addon Service)
-- 
-- ROOT CAUSE:
--   Khi Lễ tân gán dịch vụ mua thêm cho KTV đang làm đơn hiện tại, 
--   khối kiểm tra IF NOT EXISTS (status = 'ACTIVE') trả về FALSE do KTV đã có assignment ACTIVE.
--   => Dịch vụ mua thêm bị đẩy vào trạng thái QUEUED, không được đẩy vào TurnQueue.
--   => Khi KTV hoàn tất dịch vụ 1, hàm RELEASE_KTV đóng luôn dịch vụ mua thêm (vì cùng booking_id).
--   => KTV bị đẩy sang khách hàng khác (hiện nút Nhận đơn tiếp theo).
--
-- FIX:
--   Thêm điều kiện `AND "booking_id" <> p_booking_id` vào khối kiểm tra ACTIVE.
--   Nếu KTV đang ACTIVE nhưng ở CHÍNH ĐƠN HÀNG NÀY, vẫn cho phép Promote to ACTIVE 
--   và cập nhật TurnQueue để UI tự động gộp thời gian mạch lạc.
-- ============================================================================
...
BEGIN
    ...
    BEGIN
        ...
        LOOP
            ...
            -- This prevents ghost turns_completed count for reassigned orders
            -- If KTV was already working → KEEP TurnLedger (they deserve the turn credit)
