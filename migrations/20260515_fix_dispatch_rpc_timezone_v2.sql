-- ============================================================================
-- FIX: dispatch_confirm_booking (9 params) — Sửa lỗi AT TIME ZONE trừ 7 tiếng
-- 
-- ROOT CAUSE:
--   (p_date || ' ' || startTime)::timestamp AT TIME ZONE 'Asia/Bangkok' → ::time
--   converts VN local time to UTC, then extracts time → loses 7 hours.
--   Example: '19:00' becomes '12:00'.
--
-- FIX:
--   For TurnQueue.start_time / estimated_end_time (type: time without timezone):
--     → Cast directly: (elem->>'startTime')::time (no AT TIME ZONE)
--   For KtvAssignments.planned_start_time/end_time (type: timestamptz):
--     → AT TIME ZONE 'Asia/Bangkok' is CORRECT here, keep as-is.
--   For ORDER BY clauses:
--     → Use direct (elem->>'startTime')::time for consistency.
-- ============================================================================
...
-- ============================================================================
-- CLEANUP: Xóa hàm dispatch_confirm_booking(4 param) thừa đã tạo nhầm trước đó
-- ============================================================================
DROP FUNCTION IF EXISTS dispatch_confirm_booking(text, date, jsonb, jsonb);
