...
async function checkLedgerWithPagination() {
    let allBookings = [];
    let page = 0;
    const pageSize = 1000;
    ...
    const { data: ledgers } = await supabase.from('KTVDailyLedger').select('*').eq('staff_id', 'NH001');
    ...
    console.log(`Tổng Sổ Cái (NH001): ${totalLedger}`);
    console.log(`Tổng Bookings (NH001) full Pagination: ${totalBooking}`);
}

checkLedgerWithPagination().catch(console.error);
