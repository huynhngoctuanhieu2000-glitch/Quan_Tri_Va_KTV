-- Cập nhật RPC promote_next_assignment và dispatch_confirm_booking
-- Fix lỗi ghi đè trạng thái 'off' (đã tan ca) thành 'waiting' khi KTV hoàn tất đơn cuối
-- NOTE: Bản vá này được build dựa trên phiên bản code mới nhất (từ 20260609_fix_addon_service_merge.sql)

-- =================================================================================
-- 1. Cập nhật promote_next_assignment
-- =================================================================================
...
BEGIN
    ...
    IF NOT FOUND THEN
        ...
        BEGIN
            ...
            UPDATE "TurnQueue"
            SET 
                "status" = CASE WHEN "status" = 'off' THEN 'off' ELSE 'waiting' END,
                ...
                "queue_position" = CASE WHEN "status" = 'off' THEN "queue_position" ELSE v_max_pos + 1 END
...
-- =================================================================================
-- 2. Cập nhật dispatch_confirm_booking
-- =================================================================================
...
BEGIN
    ...
    -- ✅ FIX 28/07/2026: CHỈ xoá KTV thuộc về những Dịch vụ (BookingItems) ĐANG ĐƯỢC DISPATCH LẺ
    DECLARE
    ...
    BEGIN
        -- Lấy danh sách ID của các dịch vụ đang được update đợt này
        ...
        -- Lặp qua những assignment ĐANG CÓ của đơn hàng, nhưng CHỈ TẬP TRUNG vào các dịch vụ đang được update
        -- Nếu KTV không nằm trong danh sách p_staff_assignments MỚI của dịch vụ đó -> Bị xoá
        FOR v_removed_ktv_id, v_removed_item_id IN
            ...
            WHERE "booking_id" = p_booking_id
              ...
              AND NOT EXISTS (
                  -- Kiểm tra xem KTV này có được gửi lên lại cho đúng dịch vụ này không
        ...
        LOOP
            ...
            -- ✅ FIX: Check if KTV was working BEFORE clearing TurnQueue
            SELECT EXISTS(
                SELECT 1 FROM "TurnQueue"
            ...
            -- ✅ FIX: Delete TurnLedger for removed KTV if they haven't started working
            -- And ONLY IF they are not assigned to ANY OTHER active item in this same booking!
    ...
    LOOP
        ...
        -- ✅ KtvAssignments.planned_start_time là timestamptz → AT TIME ZONE đúng ở đây
        ...
        BEGIN
            ...
            IF NOT v_is_active_on_other THEN
                ...
                SET
                    ...
                    END,
                    -- 🔥 FIX CHÍNH: KHÔNG GHI ĐÈ GIỜ NẾU ĐANG WORKING
                    "start_time" = CASE 
