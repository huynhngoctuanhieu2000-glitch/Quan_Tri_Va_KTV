-- 1. Thêm cột parent_booking_id (self-referencing FK)
ALTER TABLE "Bookings" 
  ADD COLUMN IF NOT EXISTS "parent_booking_id" text REFERENCES "Bookings"("id") ON DELETE SET NULL;

-- 2. Thêm cột sub_suffix  
ALTER TABLE "Bookings" 
  ADD COLUMN IF NOT EXISTS "sub_suffix" text;

-- 3. Thêm SPLIT vào BookingStatus enum
...
-- 5. Trigger tự động cập nhật đơn cha
CREATE OR REPLACE FUNCTION fn_sync_parent_booking_status()
RETURNS TRIGGER AS $$
DECLARE
    v_parent_id text;
    v_all_done boolean;
BEGIN
    -- Chỉ xử lý đơn con (có parent_booking_id)
    IF NEW."parent_booking_id" IS NULL THEN RETURN NEW; END IF;
    
    v_parent_id := NEW."parent_booking_id";
    
    -- Check tất cả đơn con cùng cha
    SELECT bool_and("status"::text IN ('DONE', 'CANCELLED'))
    INTO v_all_done
    FROM "Bookings"
    WHERE "parent_booking_id" = v_parent_id;
    
    IF v_all_done THEN
        UPDATE "Bookings" SET "status" = 'DONE', "updatedAt" = now()
        WHERE "id" = v_parent_id AND "status"::text = 'SPLIT';
    END IF;
...
DROP TRIGGER IF EXISTS tr_sync_parent_booking_status ON "Bookings";
CREATE TRIGGER tr_sync_parent_booking_status
AFTER UPDATE OF "status" ON "Bookings"
FOR EACH ROW EXECUTE FUNCTION fn_sync_parent_booking_status();


-- 6. Cập nhật dispatch_confirm_booking (Thêm guard và cập nhật trọng số SPLIT)
