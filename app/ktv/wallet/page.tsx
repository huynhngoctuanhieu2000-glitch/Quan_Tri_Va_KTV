...
import { Zap, Clock, Banknote, TrendingDown, TrendingUp, Gift } from 'lucide-react';
            ...
            <div className="p-4 lg:p-8 space-y-6 max-w-2xl mx-auto pb-32">
                ...
                {isLoading ? (
                    ...
                    <>
                            ...
                            <div className={`p-6 rounded-[32px] shadow-lg shadow-emerald-900/10 bg-gradient-to-br from-emerald-600 to-teal-800 text-white`}>
                                ...
                                <div className="mb-5">
                                    ...
                                    <p className="text-4xl font-black tracking-tight drop-shadow-sm">
                                        {Number(walletBalance.effective_balance) - Number(walletBalance.min_deposit) > 0 
                                            ? (Number(walletBalance.effective_balance) - Number(walletBalance.min_deposit)).toLocaleString() 
                                            : 0}đ
                                    </p>
                                </div>
                        ...
                        {walletTimeline && walletTimeline.length > 0 ? (
                            <div className="bg-white rounded-3xl p-5 shadow-sm border border-slate-100">
                                ...
                                <div className="space-y-4 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-100 before:to-transparent">
                                    {walletTimeline.map((item: any, idx: number) => {
                                            ...
                                            <div key={item.id || idx} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                                                ...
                                                <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-white p-4 rounded-2xl border border-slate-100 shadow-sm transition-all hover:shadow-md">
                                                    <div className="flex items-center justify-between mb-1">
                                                        <span className="font-bold text-slate-800 text-xs line-clamp-1">{item.title}</span>
                                                        <span className={`font-black text-sm whitespace-nowrap ${isWithdrawal ? 'text-rose-600' : isPositive ? 'text-emerald-600' : 'text-rose-600'}`}>
                                                    ...
                                                    {item.note && <div className="mt-2 text-[10px] text-slate-500 bg-slate-50 p-2 rounded-lg">{item.note}</div>}
                                                </div>
                                            </div>
                                        );
                                    })}
